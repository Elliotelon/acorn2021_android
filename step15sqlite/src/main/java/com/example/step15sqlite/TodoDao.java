...
public class TodoDao {
    ...
    public void insert(TodoDto dto){
        ...
        // datetime('now','localtime')은 오라클의 SYSDATE와 비슷하다.
        String sql="INSERT INTO todo (content, regdate)"+
                "VALUES(?, datetime('now','localtime'))";
        //sql문 실행하기
        db.execSQL(sql, args);
        //close()를 호출해야 실제 반영된다.
        db.close();
    }
    ...
    public TodoDto getData(int num){
        ...
        //select 문의 수행결과는 Cursor 객체로 리턴된다. Cursor는 이클립스 JDBC에서 resultset으로 이해하면된다.
        Cursor result=db.rawQuery(sql, args);
        //할일 하나의 정보를 담을 TodoDto 객체 생성
        TodoDto dto=new TodoDto();
        dto.setNum(num);
        //select 된 row가 있다면 해당 row로 커서를 한칸 내려서
        if (result.moveToNext()){
            //커서가 위치한곳의 row에서 0번째 칼럼에 있는 문자열 얻어오기
            String content=result.getString(0);
            //커서가 위치한곳의 row에서 1번째 칼럼에 있는 문자열 얻어오기

...
class DetailActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.deleteBtn).setOnClickListener{
            //인텐트에 id라는 키값으로 담긴 정수값 얻어오기 (없으면 기본값 0 )
            val id = i.getIntExtra("id", 0)
            //알림 매니저 객체 얻어오기
            val notiManager : NotificationManagerCompat =
                   NotificationManagerCompat.from(this) //여기서 this는 DetailActivty임!
        ...
        findViewById<Button>(R.id.deleteBtn).setOnClickListener(object : View.OnClickListener{
            override fun onClick(v: View?) {
                //인텐트에 id라는 키값으로 담긴 정수값 얻어오기 (없으면 기본값 0 )
                val id = i.getIntExtra("id", 0)
                //알림 매니저 객체 얻어오기
                val notiManager : NotificationManagerCompat =
                        NotificationManagerCompat.from(this@DetailActivity)
                //알림의 아이디를 이용해서 해당 알림 삭제
                notiManager.cancel(id)
                //액티비티 종료하기
                finish()

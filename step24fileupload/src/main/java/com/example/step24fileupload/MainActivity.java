...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //사진을 출력할 ImageView의 참조값 얻어와서 필드에 저장
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.takePicBtn:
                //외부 저장장치에 접근할 권한이 켜져 있는지 여부를 상수값으로 얻어낸다.
                int permissionCheck=
            ...
            case R.id.uploadBtn:
                ...
                //웹 서버에 요청할 url 주소
                String url="http://192.168.219.108:8888/spring05/gallery/upload_android.do";
                //요청 파라미터를 Map에 담는다.
    ...
    //사진을 찍겠다는 요청 메소드
    public void requestTackPic(){
        //사진을 찍겠다는 인텐트 객체 생성하기
        Intent intent=new Intent();
        intent.setAction(MediaStore.ACTION_IMAGE_CAPTURE);
        //해당 인텐트를 받아줄 app이 있는지 확인해서(카메라 app이 있는지 여부)
        if(intent.resolveActivity(getPackageManager()) != null){
            ...
            Toast.makeText(this, "카메라 app을 설치해 주세요",
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode == RESULT_OK){//작업이 성공적으로 수행이 되었다면
            switch (requestCode){//요청의 코드값으로 분기한다.
                case 999:
                    //실행순서가 여기까지 들어온다면 사진은 성공적으로 우리가 원하는 위치에 저장이 됨
                    //저장된 이미지를 ImageView에 출력하기

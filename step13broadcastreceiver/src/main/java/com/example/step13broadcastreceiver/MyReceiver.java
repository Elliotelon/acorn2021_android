...
/*
    ...
    1.BroadcastRecevier 추상 클래스를 상속받아서 만든다.
...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        //메소드의 인자로는 Context 객체와 Intent 객체가 전달 된다.
        //Intent 객체는 이 방송 수신자 객체가 활성화 될때 전달된 Intent 객체이다.
        Toast.makeText(context, "방송수신", Toast.LENGTH_SHORT).show();

        //비행기 모드가 활성화 되었는지 여부
        boolean isActivated=intent.getBooleanExtra("state", false);

        //인자로 전달된 Context는 해당 방송수신자 객체를 직접 등록한 액티비티 홋은 서비스의 참조값
